...
1. **Скачиваем только бесплатное.** У платной программы в записи физически не
   может быть адреса установщика — только адрес страницы, где её покупают.
...
Проверка живёт в одном месте, и запись, её не прошедшая, не показывается вовсе
— не «показывается с предупреждением». Сообщения об отказе именные: «„Такая-то“
платная — у неё не может быть ссылки на установщик», «„Такая-то“: установщик на
одном домене, а разработчик на другом».
...
он есть: разбирает файл, собирает окружение, подбирает настройки графики.
...
Третий вид нужен чаще, чем кажется. У одних поставщиков адрес выпуска
одноразовый, у других меняется с версией. Зашить такой значит однажды прислать
человеку пустоту. Честнее отправить на страницу.

## Родные программы macOS

Витрина умеет и их. Окружение им не нужно, но место в библиотеке есть: человек
хочет **один список своих игр**, а не два — отдельно родные, отдельно виндовые.

Правила те же: только официальные источники, платное — ссылкой на покупку.

## Данные отдельно от кода

Каталог живёт в [`catalog/index.json`](../catalog/index.json) и **свободен для
любого использования** — как и база профилей.
...
3. В `note` сказано то, что человеку важно знать **до** установки? Античит,
   требование учётной записи, известные беды на macOS.
...
- **Обновление каталога из сети**, как у профилей.
